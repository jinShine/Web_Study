...
Phase 3: 데이터베이스      ██████████ (17~25)   ⬜ 예정
Phase 4: 실전 심화        ██████████ (26~31)   ⬜ 예정
Phase 5: 배포 & 운영      ██████████ (32~38)   ⬜ 예정
Phase 6: 미니 프로젝트     ██████████ (39~43)   ⬜ 예정
```
...
| 19 | JDBC와 Spring JDBC | DataSource, JdbcTemplate, RowMapper | ⬜ |
...
| # | 주제 | 핵심 키워드 | 상태 |
|---|---|---|---|
| 26 | 인증/인가 기초 | SecurityFilterChain, 세션 vs JWT, CORS | ⬜ |
| 27 | JWT 인증 구현 | Access Token, Refresh Token, Filter | ⬜ |
| 28 | 테스트 코드 작성 | JUnit 5, Mockito, WebMvcTest, SpringBootTest | ⬜ |
| 29 | 캐싱과 성능 | @Cacheable, Redis, TTL, Eviction | ⬜ |
| 30 | 비동기와 이벤트 기반 설계 | @Async, EventListener, CompletableFuture | ⬜ |
| 31 | 외부 API 연동 | RestClient, WebClient, Retry, Circuit Breaker | ⬜ |
...
| # | 주제 | 핵심 키워드 | 상태 |
|---|---|---|---|
| 32 | Git 브랜치 전략과 협업 | Git Flow, PR, Commit Convention | ⬜ |
| 33 | Docker 기초 | Dockerfile, Compose, 이미지 빌드 | ⬜ |
| 34 | CI/CD 파이프라인 | GitHub Actions, Build, Test, Deploy | ⬜ |
| 35 | 클라우드 배포 | AWS EC2, RDS, 환경변수, 보안 그룹 | ⬜ |
| 36 | 모니터링과 로그 관리 | Actuator, Prometheus, Grafana | ⬜ |
| 37 | 서버 최적화와 유지보수 | 커넥션 풀, JVM, 부하 테스트, 프로파일링 | ⬜ |
| 38 | API 보안 체크리스트 | HTTPS, Rate Limiting, OWASP Top 10 | ⬜ |
...
| # | 주제 | 설명 | 상태 |
|---|---|---|---|
| 39 | 프로젝트 설계 | 요구사항 분석, ERD, API 명세서 | ⬜ |
| 40 | 게시판 API 구현 | CRUD, 검색, 페이징, 파일 첨부 | ⬜ |
| 41 | 인증 시스템 통합 | 회원가입, 로그인, 권한 처리 | ⬜ |
| 42 | 테스트 & 문서화 | 테스트 코드, Swagger, README | ⬜ |
| 43 | 배포 & 회고 | Docker, CI/CD, 회고 정리 | ⬜ |
