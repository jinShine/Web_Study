...
Phase 2: API 개발         ██████████ (07~16)   ← 지금 여기
Phase 3: 데이터베이스      ██████████ (17~24)
Phase 4: 실전 심화        ██████████ (25~30)
Phase 5: 배포 & 운영      ██████████ (31~37)
Phase 6: 미니 프로젝트     ██████████ (38~42)
```
...
## Phase 2: API 개발 — REST API 제대로 만들기
...
| 16 | **디버깅 완벽 가이드** | IntelliJ 디버거, Breakpoint, Step Over/Into, Watch, 조건부 BP, Remote Debug | ⬜ |
...
| 18 | **SQL 심화** | 서브쿼리, GROUP BY/HAVING, 윈도우 함수, 실행 계획, 인덱스 최적화 | ⬜ |
| 19 | **JDBC와 Spring JDBC** | DataSource, JdbcTemplate, RowMapper, 트랜잭션 기초 | ⬜ |
| 20 | **JPA 기초** | @Entity, @Id, @GeneratedValue, EntityManager, 영속성 컨텍스트 | ⬜ |
| 21 | **Spring Data JPA** | JpaRepository, 쿼리 메서드, @Query, 페이징/정렬, Pageable | ⬜ |
| 22 | **연관 관계 매핑** | @OneToMany, @ManyToOne, @ManyToMany, Fetch 전략, N+1 문제 | ⬜ |
| 23 | **QueryDSL** | BooleanExpression, Q클래스, 동적 쿼리, 프로젝션, 서브쿼리 | ⬜ |
| 24 | **트랜잭션과 동시성** | @Transactional, isolation level, propagation, 낙관적/비관적 락 | ⬜ |
...
| # | 주제 | 핵심 키워드 | 상태 |
|---|------|------------|------|
| 25 | **인증/인가 기초** | Spring Security, SecurityFilterChain, 세션 vs 토큰, CORS | ⬜ |
| 26 | **JWT 인증 구현** | JWT 구조, Access/Refresh Token, 토큰 저장 전략, Filter 구현 | ⬜ |
| 27 | **테스트 코드 작성** | JUnit 5, @SpringBootTest, @WebMvcTest, Mockito, AssertJ | ⬜ |
| 28 | **캐싱과 성능** | @Cacheable, Redis 기초, 캐시 전략(TTL, Eviction) | ⬜ |
| 29 | **비동기와 스케줄링** | @Async, @Scheduled, CompletableFuture, 이벤트 기반 처리 | ⬜ |
| 30 | **외부 API 연동** | RestTemplate, RestClient, WebClient, Feign Client, 타임아웃, 재시도 | ⬜ |
...
| # | 주제 | 핵심 키워드 | 상태 |
|---|------|------------|------|
| 31 | **Git 브랜치 전략과 협업** | Git Flow, PR, 코드 리뷰, .gitignore, 커밋 컨벤션 | ⬜ |
| 32 | **Docker 기초** | Dockerfile, docker-compose, 이미지 빌드, 컨테이너 실행 | ⬜ |
| 33 | **CI/CD 파이프라인** | GitHub Actions, 빌드→테스트→배포 자동화 | ⬜ |
| 34 | **클라우드 배포** | AWS EC2/RDS, 환경변수 관리, 보안 그룹 설정 | ⬜ |
| 35 | **모니터링과 로그 관리** | Actuator, Prometheus, Grafana, 로그 수집 전략 | ⬜ |
| 36 | **서버 최적화와 유지보수** | 커넥션 풀, 쿼리 튜닝, JVM 메모리, 프로파일링, 부하 테스트 | ⬜ |
| 37 | **API 보안 체크리스트** | HTTPS, Rate Limiting, SQL Injection 방지, OWASP Top 10 | ⬜ |
...
| # | 주제 | 설명 | 상태 |
|---|------|------|------|
| 38 | **프로젝트 설계** | 요구사항 분석, ERD 설계, API 명세서 작성 | ⬜ |
| 39 | **게시판 API 구현** | CRUD + 검색 + 페이징 + 파일 첨부 | ⬜ |
| 40 | **인증 시스템 통합** | 회원가입/로그인/권한 관리 적용 | ⬜ |
| 41 | **테스트 & 문서화** | 단위/통합 테스트, Swagger 문서, README 정리 | ⬜ |
| 42 | **배포 & 회고** | Docker + CI/CD로 배포, 프로젝트 회고록 작성 | ⬜ |
...
| Spring 고급 (15일) | 03, 05, 06, 10~16, 25~30 | Lombok, MapStruct, Security, 테스트, 문서화 등 |
| DB와 ORM (Day 1~9) | 17~24 | SQL, JPA, QueryDSL, 트랜잭션 |
| 웹 개발 심화 (4일) | 31, 33, 36 | 협업, CI/CD, 서버 최적화 |
| RESTful API 설계 (5일) | 07~08, 16 | REST + MVC + 디버깅 |
| 클라우드 배포 (8일) | 34 | AWS EC2/RDS |
...
> 🔄 2026-03-05: 부트캠프 커리큘럼 병합 완료 — 7개 주제 추가 (총 35 → 42)
